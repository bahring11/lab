...
static float DEFAULT_TRANSLATE_SPEED = 0.5f;
static float DEFAULT_ZOOM_FRACTION = 2.5f;
static float DEFAULT_ROTATE_SPEED = 2.0f;
...
void Viewer::zoom(float changeVert) {
    float scaleFactor = powf(2.0f, -changeVert * m_zoomFraction);
    m_viewPoint = m_viewCenter + (m_viewPoint - m_viewCenter) * scaleFactor;
    getFrustrumInfo();
}

// ÄõÅÍ´Ï¾ð È¸Àü ÇïÆÛ ÇÔ¼ö
...
void makeOrthogonalTo(glm::vec3& vec1, glm::vec3& vec2) {
    ...
    if (length == 0) {
        std::cout << "Warning: called makeOrthogonalTo with zero vector." << std::endl;
    }
    else {
...
void Viewer::rotate(float changeHoriz, float changeVert) {
    ...
    glm::quat horizRot = setFromAxisAngle(m_imagePlaneHorizDir, horizRotAngle);
    glm::quat vertRot = setFromAxisAngle(m_imagePlaneVertDir, vertRotAngle);
...
void Viewer::getFrustrumInfo() {
    ...
    m_displayHeight = 2.0f * glm::length(m_viewCenter - m_viewPoint) * tan(glm::radians(0.5f * m_fieldOfView));
